     ...
     <div className="h-full overflow-y-scroll p-6 flex items-start gap-6 text-slate-700">
      ...
      <form onSubmit={onSubmitHandler} className="flex-1 max-w-xl p-4 bg-white rounded-lg border border-gray-200">
        ...
        <textarea 
          ...
          placeholder="describe what you want see in the image"
          required 
        />

        <p className="mt-4 text-sm font-medium">Style </p>
        <div className="mt-3 flex gap-3 flex-wrap">
          {imageStyle.map((items)=>(
            <span 
              onClick={()=>setselectedStyle(items)} 
              className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${
                selectedStyle === items ? 'bg-green-50 text-green-700 border-blue-400' : 'text-gray-500 border-gray-300'
              }`} 
              key={items}
            >
              {items}
        ...
        <div className='my-6 flex-items-center gap-2'>
              ...
              <p className='text-sm'>Make this image Publich</p>
        </div>
