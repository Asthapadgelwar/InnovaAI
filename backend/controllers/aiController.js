...
export const generateArticle = async (req, res) => {
  try {
    ...
    // Fetch current free usage from Clerk
    ...
    if (response?.response?.candidates?.[0]?.content?.parts?.[0]?.text) {
      ...
      content = response.response.output_text;
    }

    // ✅ Ensure not null/empty
    ...
    // ✅ Cut down to requested length (approx. word count)
    if (length) {
      const words = content.split(" ");
      content = words.slice(0, length).join(" ");
    }
...
export const generateBlogTitle = async (req, res) => {
  try {
    ...
    // Optionally cut down to requested length (approx. word count)
    if (length) {
      const words = content.split(" ");
      content = words.slice(0, length).join(" ");
    }
